...
function Request({ onClose }) {
  ...
  useEffect(() => {
    if (location.state) {
      setFormValues((prevValues) => ({
        ...prevValues,
        mainName: location.state.tenantName || "",
        name: location.state.tenantName || "",
        email: location.state.email || "",
        phoneNumber: location.state.tenantPhone || "",
        houseNumber: location.state.houses?.estate[0]?.houseNumber || "",
        street: "",
        totalAmount: serviceCharge * requests.length,
      }));
    }
  }, [location.state, requests.length, serviceCharge]);
  ...
  const addRequest = () => {
    ...
    const newRequest = {
      ...
      passport: media, // Store the uploaded file in the request
    ...
    setRequests((prevRequests) => {
      ...
      setFormValues((prevValues) => ({
        ...prevValues,
        totalAmount: serviceCharge * updatedRequests.length,
      }));
      ...
      <Box px={["30px", "50px"]}>
        <FormControl h="100%" my="30px" color={"black"}>
          <Stack spacing="20px">
            ...
            >
              Fill in your details and upload passport photo to request for a
              new ID card
            </Text>
          ...
          <Box my="30px" color={"#000"}>
            ...
            {requests.map((req, index) => (
              <Box key={index} bg="#f7f7f7" p="10px" borderRadius="8px">
                <Text mb="5px">
                  Name: {req.name}, Type: {req.type}, Email: {req.email}, Phone:{" "}
                  {req.phoneNumber}, House Number: {req.houseNumber}, Street:{" "}
                  {req.street}, Total Amount: N{serviceCharge}
                </Text>
        ...
        >
          ...
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              Please fill in your details in the form and upload passport before proceeding.
            </AlertDialogBody>
            <AlertDialogFooter>
